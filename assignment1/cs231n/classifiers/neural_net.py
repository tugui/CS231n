...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    dlog = 1 / (np.exp(correct_class_scores) / sum_score)
    db2 = -1 * dlog * coef
    grads['b2'] = np.sum(db2, axis=0) / num_train

    dW2 = np.dot(output.T, db2)
    dW2 /= num_train
    dW2 += reg * 2 * W2
    grads['W2'] = dW2

    doutput = np.dot(db2, W2.T)
    db1 = doutput * np.sign(output)
    grads['b1'] = np.sum(db1, axis=0) / num_train
  ...
  def predict(self, X):
    ...
    ###########################################################################
    output = np.maximum(np.dot(X, self.params['W1']) + self.params['b1'], 0)
    scores = np.dot(output, self.params['W2']) + self.params['b2']
