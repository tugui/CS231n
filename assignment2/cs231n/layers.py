...
def relu_backward(dout, cache):
    ...
    ###########################################################################
    dx = dout * np.sign(np.maximum(x, 0))
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        tmp = (x - sample_mean) / np.sqrt(sample_var)
        out = tmp * gamma + beta
        running_mean = momentum * running_mean + (1 - momentum) * sample_mean
        running_var = momentum * running_var + (1 - momentum) * sample_var
...
def batchnorm_backward_alt(dout, cache):
    ...
    ###########################################################################
    """
    dx = (N - 1) / N * np.sqrt(cache['var']) - np.square(cache['x'] - cache['mean']) / (N * np.sqrt(cache['var']))
    dx /= cache['var']
    dx *= cache['gamma'] * dout
    """
    ###########################################################################
    ...
    for n in range(filter_num):
        for i in range(output_height):
            for j in range(output_width):
                out[:, n, i, j] = np.sum(w[n] * ex[:, :, stride*i : stride*i + filter_height, stride*j : stride*j + filter_width], axis=(1, 2, 3))
        out[:, n, :, :] += b[n]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def conv_backward_naive(dout, cache):
    ...
    tmp = np.zeros((train_num, channel_num, filter_height, filter_width))
    edx = np.zeros((x.shape[0], x.shape[1], x.shape[2] + 2 * pad, x.shape[3] + 2 * pad))
    for n in range(filter_num):
        for i in range(output_height):
            for j in range(output_width):
                for t in range(train_num):
                    tmp[t, ...] = ex[t, :, stride*i : stride*i + filter_height, stride*j : stride*j + filter_width] * dout[t, n, i, j]
                    edx[t, :, stride*i : stride*i + filter_height, stride*j : stride*j + filter_width] += w[n] * dout[t, n, i, j]
                dw[n] += np.sum(tmp, axis=0)
    dx = edx[:, :, pad:x.shape[2] + pad, pad:x.shape[3] + pad]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx, dw, db
...
def max_pool_forward_naive(x, pool_param):
    ...
    out = np.zeros((train_num, channel_num, output_height, output_width))
    for t in range(train_num):
        for c in range(channel_num):
            for i in range(output_height):
                for j in range(output_width):
                    out[t, c, i, j] = np.max(x[t, c, stride*i : stride*i + pool_height, stride*j : stride*j + pool_width])
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, pool_param)
...
def max_pool_backward_naive(dout, cache):
    ...
    for t in range(train_num):
        for c in range(channel_num):
            for i in range(output_height):
                for j in range(output_width):
                    dx[t, c, stride*i : stride*i + pool_height, stride*j : stride*j + pool_width] = (x[t, c, stride*i : stride*i + pool_height, stride*j : stride*j + pool_width] == np.max(x[t, c, stride*i : stride*i + pool_height, stride*j : stride*j + pool_width])) * dout[t, c, i, j]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx
